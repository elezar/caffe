...
inline __device__ Dtype backward_i(const Dtype a, const Dtype s, const Dtype t) {
  return a * (s - t);
}

template <typename Dtype>
__global__ void backward_with_ignore_kernel(const int n,
    const Dtype ignore_label, const Dtype a,
    const Dtype* __restrict__ target, const Dtype* __restrict__ sigmoid,
    Dtype* __restrict__ diff) {
  CUDA_KERNEL_LOOP(index, n) {
    diff[index] = (ignore_label != target[index]) * backward_i(a, sigmoid[index], target[index]);
  }
}

template <typename Dtype>
__global__ void backward_kernel(const int n,
    const Dtype a,
    const Dtype* __restrict__ target, const Dtype* __restrict__ sigmoid,
    Dtype* __restrict__ diff) {
  CUDA_KERNEL_LOOP(index, n) {
  ...
  if (propagate_down[0]) {
    ...
    if (has_ignore_label_) {
      ...
      backward_kernel<<<grid_size, num_threads>>>(
          count, scale, target, sigmoid_output_data,
          bottom_diff);
    }
